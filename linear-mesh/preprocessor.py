...
class Preprocessor:
    ...
    def preprocess(self, signal, stas_legacy, stas_ax):
        window = 150
        res = []

        for i in range(0, len(signal), window//2):
            if stas_legacy == -1: # use p_col as state
                res.append([
                ...
                res.append([
                    ...
                    stas_legacy, stas_ax] for batch in range(0, signal.shape[1])])
        res = np.array(res)
        if stas_legacy == -1:
            res = np.clip(res, 0, 1)
        else:
            res = np.clip(res, 0, max(stas_legacy, stas_ax))
